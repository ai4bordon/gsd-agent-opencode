---
name: gsd-update
description: Обновить GSD до последней версии с показом changelog
subtask: true
---
...
Provides a better update experience than raw `npx gsd-agent-opencode` by showing version diff and changelog entries.
</objective>

<process>

<step name="get_installed_version">
read installed version (local install has priority):
...
```bash
npm view gsd-agent-opencode version 2>/dev/null
```
...
To update manually: `npx gsd-agent-opencode@latest --global`
```
...
```bash
# Обновление = переустановка. Выбираем scope автоматически.
if [ -d ./.opencode/get-shit-done ]; then
  npx gsd-agent-opencode@latest --local --force
else
  npx gsd-agent-opencode@latest --global --force
fi
```
