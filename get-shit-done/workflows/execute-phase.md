...
These settings are used for retry loops on autonomous plans only.
</step>
...
2. **Execute the wave with RALF Lite retry rounds for autonomous plans:**
   ...
   **Round strategy (per wave):**
   - Start with all incomplete plans in the wave.
   - Spawn remaining plans in parallel.
   - For each plan result:
     - success (`SUMMARY.md` exists): mark complete, remove from remaining list
     - failure on `autonomous: true` and `RALF_ENABLED=true` and `RALF_AUTONOMOUS_PLANS=true`: candidate for retry
     - failure on checkpoint plans (`autonomous: false`): route through checkpoint flow, no RALF retry
   - Retry only failed autonomous plans until stop conditions trigger.

   **RALF stop conditions at plan level:**
   - success: plan completed (`SUMMARY.md` created)
   - exhausted: reached `RALF_MAX_ITERATIONS`
   - repeated fingerprint: same failure signature repeated `RALF_ERROR_REPEAT_LIMIT`
   - no progress: unchanged failure/progress repeated `RALF_NO_PROGRESS_LIMIT`

   **Prompt for each spawned plan (per round):**
   ...
   <execution_context>
   @gsd-agent-opencode/get-shit-done/workflows/execute-plan.md
   @gsd-agent-opencode/get-shit-done/templates/summary.md
   @gsd-agent-opencode/get-shit-done/references/checkpoints.md
   @gsd-agent-opencode/get-shit-done/references/tdd.md
   @gsd-agent-opencode/get-shit-done/references/ralf-lite.md
   </execution_context>
   ...
   RALF round:
   - attempt: {attempt}
   - max: {RALF_MAX_ITERATIONS}
   - previous_failure_fingerprint: {if any}
   </context>
...
3. **Wait for all agents in the round to complete:**

   Task tool blocks until each agent finishes. All parallel agents in the round return together.
...
5. **Handle failures:**
   ...
   If a plan still fails after RALF stop conditions:
   - Report which plan failed and why
   - Include attempts used and last fingerprint
...
### RALF Lite Outcome
[Aggregate retries, exhausted plans, and stop reasons from plan summaries]
```
</step>
...
- For autonomous plans, apply RALF retry rounds until stop condition
- If still failing, report attempts + fingerprint and ask user how to proceed
