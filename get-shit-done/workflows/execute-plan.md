...
@gsd-agent-opencode/get-shit-done/references/git-integration.md
@gsd-agent-opencode/get-shit-done/references/ralf-lite.md
</required_reading>
...
<step name="load_ralf_lite_config">
Resolve effective RALF Lite settings (global defaults + project overrides).

**Default values:**
- `RALF_ENABLED=true`
- `RALF_MAX_ITERATIONS=3`
- `RALF_ERROR_REPEAT_LIMIT=2`
- `RALF_NO_PROGRESS_LIMIT=2`
- `RALF_AUTO_TASKS=true`
- `RALF_AUTONOMOUS_PLANS=true`
- `RALF_TDD_TASKS=false`

```bash
# Resolve global config location
GLOBAL_OPENCODE_DIR="${OPENCODE_CONFIG_DIR:-$HOME/.config/opencode}"
GLOBAL_GSD_CONFIG="$GLOBAL_OPENCODE_DIR/get-shit-done/config.json"
...
# Global defaults (if file exists)
if [ -f "$GLOBAL_GSD_CONFIG" ]; then
  G_ENABLED=$(grep -A 30 '"ralf_lite"' "$GLOBAL_GSD_CONFIG" | grep -m1 -o '"enabled"[[:space:]]*:[[:space:]]*[^,}]*' | grep -o 'true\|false')
  G_MAX=$(grep -A 30 '"ralf_lite"' "$GLOBAL_GSD_CONFIG" | grep -m1 -o '"max_iterations"[[:space:]]*:[[:space:]]*[0-9]\+' | grep -o '[0-9]\+')
  G_ERR=$(grep -A 30 '"ralf_lite"' "$GLOBAL_GSD_CONFIG" | grep -m1 -o '"error_fingerprint_repeats"[[:space:]]*:[[:space:]]*[0-9]\+' | grep -o '[0-9]\+')
  G_NOPROG=$(grep -A 30 '"ralf_lite"' "$GLOBAL_GSD_CONFIG" | grep -m1 -o '"no_progress_repeats"[[:space:]]*:[[:space:]]*[0-9]\+' | grep -o '[0-9]\+')
  G_AUTO=$(grep -A 30 '"apply_to"' "$GLOBAL_GSD_CONFIG" | grep -m1 -o '"auto_tasks"[[:space:]]*:[[:space:]]*[^,}]*' | grep -o 'true\|false')
  G_AUTOPLAN=$(grep -A 30 '"apply_to"' "$GLOBAL_GSD_CONFIG" | grep -m1 -o '"autonomous_plans"[[:space:]]*:[[:space:]]*[^,}]*' | grep -o 'true\|false')
  G_TDD=$(grep -A 30 '"apply_to"' "$GLOBAL_GSD_CONFIG" | grep -m1 -o '"tdd_tasks"[[:space:]]*:[[:space:]]*[^,}]*' | grep -o 'true\|false')

  [ -n "$G_ENABLED" ] && RALF_ENABLED="$G_ENABLED"
  [ -n "$G_MAX" ] && RALF_MAX_ITERATIONS="$G_MAX"
  [ -n "$G_ERR" ] && RALF_ERROR_REPEAT_LIMIT="$G_ERR"
  [ -n "$G_NOPROG" ] && RALF_NO_PROGRESS_LIMIT="$G_NOPROG"
  [ -n "$G_AUTO" ] && RALF_AUTO_TASKS="$G_AUTO"
  [ -n "$G_AUTOPLAN" ] && RALF_AUTONOMOUS_PLANS="$G_AUTOPLAN"
  [ -n "$G_TDD" ] && RALF_TDD_TASKS="$G_TDD"
fi

# Project overrides
if [ -f .planning/config.json ]; then
  L_ENABLED=$(grep -A 30 '"ralf_lite"' .planning/config.json | grep -m1 -o '"enabled"[[:space:]]*:[[:space:]]*[^,}]*' | grep -o 'true\|false')
  L_MAX=$(grep -A 30 '"ralf_lite"' .planning/config.json | grep -m1 -o '"max_iterations"[[:space:]]*:[[:space:]]*[0-9]\+' | grep -o '[0-9]\+')
  L_ERR=$(grep -A 30 '"ralf_lite"' .planning/config.json | grep -m1 -o '"error_fingerprint_repeats"[[:space:]]*:[[:space:]]*[0-9]\+' | grep -o '[0-9]\+')
  L_NOPROG=$(grep -A 30 '"ralf_lite"' .planning/config.json | grep -m1 -o '"no_progress_repeats"[[:space:]]*:[[:space:]]*[0-9]\+' | grep -o '[0-9]\+')
  L_AUTO=$(grep -A 30 '"apply_to"' .planning/config.json | grep -m1 -o '"auto_tasks"[[:space:]]*:[[:space:]]*[^,}]*' | grep -o 'true\|false')
  L_AUTOPLAN=$(grep -A 30 '"apply_to"' .planning/config.json | grep -m1 -o '"autonomous_plans"[[:space:]]*:[[:space:]]*[^,}]*' | grep -o 'true\|false')
  L_TDD=$(grep -A 30 '"apply_to"' .planning/config.json | grep -m1 -o '"tdd_tasks"[[:space:]]*:[[:space:]]*[^,}]*' | grep -o 'true\|false')

  [ -n "$L_ENABLED" ] && RALF_ENABLED="$L_ENABLED"
  [ -n "$L_MAX" ] && RALF_MAX_ITERATIONS="$L_MAX"
  [ -n "$L_ERR" ] && RALF_ERROR_REPEAT_LIMIT="$L_ERR"
  [ -n "$L_NOPROG" ] && RALF_NO_PROGRESS_LIMIT="$L_NOPROG"
  [ -n "$L_AUTO" ] && RALF_AUTO_TASKS="$L_AUTO"
  [ -n "$L_AUTOPLAN" ] && RALF_AUTONOMOUS_PLANS="$L_AUTOPLAN"
  [ -n "$L_TDD" ] && RALF_TDD_TASKS="$L_TDD"
fi
```

Record effective values for Summary reporting.
</step>
...
2. For each task:
   ...
   **Before executing:** Check if task has `tdd="true"` attribute:
   - If yes and `RALF_TDD_TASKS=false`: Follow TDD execution flow (see `<tdd_execution>`) with no RALF loop
   - Otherwise: run RALF Lite task loop when enabled

   **RALF Lite task loop (`type="auto"`):**
   - Activate only when `RALF_ENABLED=true` and `RALF_AUTO_TASKS=true`
   - Maximum attempts: `RALF_MAX_ITERATIONS` (default 3)
   - Per iteration:
     1. Work toward task completion
     2. **If CLI/API returns authentication error:** Handle authentication gate (see below)
     3. **When you discover additional work not in plan:** Apply deviation rules automatically
     4. Run task verification (`<verify>`) and check `<done>` criteria
     5. Capture failure fingerprint from verify output (first stable error signature)
     6. Compare with prior iterations for repeated fingerprint and no-progress
   - Stop loop on success:
     - verification passes and done criteria met
   - Stop loop on failure:
     - attempts exhausted (`RALF_MAX_ITERATIONS`)
     - same fingerprint repeated `RALF_ERROR_REPEAT_LIMIT`
     - no-progress repeated `RALF_NO_PROGRESS_LIMIT`

   **When loop succeeds:**
   - **Commit the task** (see `<task_commit>` below)
   - Track task completion, commit hash, attempts used, and stop reason for Summary
   - Continue to next task

   **When loop exhausts:**
   - STOP current plan execution
   - Return checkpoint (decision) with:
     - attempts used
     - fingerprint history
...
**Include `RALF Lite Loop Report` section:**

- Effective config (enabled, max iterations, repeat limits, apply_to)
- Attempts used per auto task
- Stop reason per task: `success`, `max_iterations`, `repeated_fingerprint`, `no_progress`
- Aggregate stats for this plan (total looped tasks, total retries, exhausted tasks)
  </step>
...
- RALF Lite Loop Report included in SUMMARY.md for auto tasks
