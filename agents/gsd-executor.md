...
Use these values for auto-task retry loops and summary reporting.
</step>
...
2. **If `type="auto"`:**
   ...
   - Check if task has `tdd="true"` attribute
   - If `tdd="true"` and `RALF_TDD_TASKS=false` → follow TDD execution flow without RALF loop
   - Otherwise, if `RALF_ENABLED=true` and `RALF_AUTO_TASKS=true`, run bounded RALF loop:
     1. Attempt implementation and verification
     2. Handle auth gates and deviation rules as normal
     3. Capture failure fingerprint and progress signal
     4. Retry until success or stop condition
   - Stop conditions:
     - success: verify + done criteria satisfied
     - exhausted: attempts reach `RALF_MAX_ITERATIONS`
     - repeated fingerprint: same failure repeated `RALF_ERROR_REPEAT_LIMIT`
     - no progress: unchanged outcome repeated `RALF_NO_PROGRESS_LIMIT`
   - On success: **Commit the task** (see task_commit_protocol)
   - Track completion, commit hash, attempts used, and stop reason for Summary
   - On exhaustion: return structured checkpoint (decision) with retry diagnostics
...
**See @gsd-agent-opencode/get-shit-done/references/checkpoints.md**
...
**Use template from:** @gsd-agent-opencode/get-shit-done/templates/summary.md
...
```markdown
## RALF Lite Loop Report

- Enabled: {true|false}
- Effective config: max_iterations={N}, error_fingerprint_repeats={N}, no_progress_repeats={N}
- Auto tasks looped: {count}
- Total retries: {count}

### Task Attempts

| Task | Attempts | Stop Reason |
|------|----------|-------------|
| 1    | 1        | success |
| 2    | 3        | repeated_fingerprint |
```
...
- [ ] SUMMARY.md includes RALF Lite loop report for auto tasks
